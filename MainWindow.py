...
class MyMainWindow(QtWidgets.QWidget):
 ...
 def enterValues(self):
         ...
         if self.editA.text().isdigit() and self.editB.text().isdigit() and self.editC.text().isdigit() and self.editD.text().isdigit() and self.editE.text().isdigit():
             self.datasource.orderStreams.append({"Header": self.counter+1, "Lines": [{"Product": "A", "Quantity": int(self.editA.text())},
...
# This method gets called when user presses reset button
 def reset(self):
     ...
     result = QtWidgets.QMessageBox.question( self, 'Question', "Would you like to process orders in Backordered Queue?", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,QtWidgets.QMessageBox.No)
